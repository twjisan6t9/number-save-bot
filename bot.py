...
async def login(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not acc:
        await update.message.reply_text(f"❌ {phone} list-e nai! Aage /addnumber diye add koro!")
        return
    client = TelegramClient(StringSession(), API_ID, API_HASH)
    await client.connect()
    await client.send_code_request(phone)
    login_sessions[update.effective_user.id] = {"client": client, "phone": phone}
    await update.message.reply_text(f"📱 {phone} te OTP pathano hoyeche!\n\nCode dao:")
    return WAITING_CODE
...
async def get_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not session_data:
        await update.message.reply_text("❌ Aage /login diye shuru koro!")
